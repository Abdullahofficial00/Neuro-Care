...
# Mongo db connection path in .env file
...
def search_patient():
    ...
    if not patient_id:
        return jsonify({'exists': False, 'message': 'No patient ID provided'}), 400

    # Cast to string for consistent comparison
    patient = patients_collection.find_one({"patientId": str(patient_id)})

    if patient:
        patient['_id'] = str(patient['_id'])  # Convert ObjectId to string for JSON
        return jsonify({'exists': True, 'patient': patient})
    else:
        return jsonify({'exists': False, 'message': 'Patient not found'})
