...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function searchPatient(patientId) {
        ...
        .then(data => {
            if (data.exists) {
                // Show patient data on UI
                console.log('Patient found:', data.patient);
                // TODO: populate patient details section with data.patient
            } else {
                alert('Patient not found. Please create new user.');
                // Optionally redirect or show create user form
            }
        })
        .catch(err => alert('Error searching patient: ' + err.message));
    }
...
document.getElementById('searchBtn').addEventListener('click', () => {
  ...
  .then(data => {
    if (data.exists) {
      alert('Patient found: ' + JSON.stringify(data.patient));
      // Optionally fill form or show patient info somewhere
      document.getElementById('patientForm').style.display = 'none';
      // You can add code to display patient data in some div if needed
    } else {
      alert('Patient not found. You can create a new user.');
      document.getElementById('patientForm').style.display = 'none';
    }
  })
  .catch(err => alert('Error searching patient: ' + err.message));
...
document.getElementById('patientFormData').addEventListener('submit', e => {
  e.preventDefault();

  const patientId = document.getElementById('searchInput').value.trim();
  if (!patientId) {
    alert('Please enter Patient ID in search box before submitting.');
    return;
  }
  ...
  const patientData = {
    ...
    caregiver: document.getElementById('caregiverName').value.trim(),
